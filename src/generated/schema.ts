                    ...
                    "application/json": {
                        ...
                        data?: {
                            ...
                            validationResult: {
                                ...
                                rulesetChannel?: "latest" | "previous";
                                /** @description True when `previous` was requested and the format had nothing for the channel to reach, so the request ran what `latest` would have run. Either nothing is retained, or what is retained has been superseded by the authority making its successor mandatory. A superseded version is still reachable by an exact pin. Present only alongside `rulesetChannel`. */
                                rulesetFellBack?: boolean;
                                /**
                                 * Format: date
                                 * @description ISO date the retained ruleset serving this request stops being served. Present only when a retained ruleset ran (never on a default request); the earliest date when several ran. After it an exact pin 422s. It bounds the pin, not the channel: `previous` stops reaching the version on this date or on the authority's mandatory date for the successor, whichever comes first.
                                 */
                                rulesetSunsetsOn?: string;
                                errors: {
                                    ruleId: string;
                    ...
                    "application/json": {
                        ...
                        data?: {
                            ...
                            scheduledChanges: {
                                ...
                                effectiveOn: string;
                                /** @description ISO 8601 date (YYYY-MM-DD) the change was announced in the public ruleset changelog, or null when it has not been recorded yet. */
                                noticePublishedOn: string | null;
                                /** @description ISO 8601 date (YYYY-MM-DD) after which `from` is no longer served at all, or null when nothing retires it. Later than `effectiveOn`, and the date to act on: it is when an exact pin on `from` starts returning a 422. It does not bound the `previous` channel, which stops resolving to `from` on this date or on the authority's mandatory date for `to`, whichever comes first. */

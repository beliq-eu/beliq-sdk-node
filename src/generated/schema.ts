                    ...
                    "application/json": {
                        ...
                        data?: {
                            ...
                            validationResult: {
                                ...
                                /**
                                 * Format: date
                                 * @description ISO date the retained ruleset serving this request stops being served. Present only when a retained ruleset ran (never on a default request); the earliest date when several ran. After it, `previous` falls back to `latest` and an exact pin 422s.
                                 */
                    ...
                    "application/json": {
                        ...
                        data?: {
                            ...
                            scheduledChanges: {
                                ...
                                /** @description ISO 8601 date (YYYY-MM-DD) after which `from` is no longer served at all, or null when nothing retires it. Later than `effectiveOn`, and the date to act on: it is when a `Beliq-Ruleset: previous` pin stops resolving to `from` and an exact pin on it starts returning a 422. */
